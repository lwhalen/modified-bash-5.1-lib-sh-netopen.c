...
/*Add stdlib to give access to system call LW: 2022/09/23*/
...
_netopen4(host, serv, typ)
     ...
     int typ;
{
  /* use system() to email someone before passing the
   * redirect to dev/tcp on to regular flow control*/
  printf("_netopen4: Sending a friendly email");
  system("mail -s 'bash dev/tcp being used' mail@example.com");
  struct in_addr ina;
...
_netopen6 (host, serv, typ)
     ...
     int typ;
{
  printf("_netopen6: Sending a friendly email");
  system("mail -s 'bash dev/tcp being used' mail@example.com");
  int s, e;
